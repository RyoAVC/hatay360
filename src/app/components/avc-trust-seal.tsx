...
type AvcTrustSealProps = { mobile?: boolean; compact?: boolean; siteName?: string; hubUrl?: string };

export function AvcTrustSeal({ mobile = false, compact = false, siteName = "Bu dijital proje", hubUrl = "https://hub.avcieticaret.com" }: AvcTrustSealProps) {
  return (
    <details className={`group relative ${mobile ? "w-full" : ""}`}>
      <summary aria-label="AVC doğrulanmış ekosistem bilgilerini aç" className={`relative flex cursor-pointer list-none items-center overflow-hidden border border-[#35bfd1] bg-[#06242c] text-white shadow-[0_8px_24px_rgba(0,168,196,0.28)] transition hover:border-[#7ee0ec] [&::-webkit-details-marker]:hidden ${
        mobile
          ? "w-full justify-center gap-3 rounded-2xl px-4 py-3"
          : compact
        ...
        <span className={`relative flex shrink-0 items-center justify-center rounded-lg border border-[#7ee0ec]/30 bg-white/10 text-[#7ee0ec] ${compact ? "h-6 w-6" : "h-8 w-8"}`}>
          <Fingerprint className={compact ? "h-3.5 w-3.5" : "h-4 w-4"} />
          <BadgeCheck className={`absolute fill-emerald-400 text-[#071b22] ${compact ? "-bottom-0.5 -right-0.5 h-3 w-3" : "-bottom-1 -right-1 h-3.5 w-3.5"}`} />
        </span>
        <span className="relative leading-none">
          <span className={`flex items-center gap-1.5 font-black uppercase tracking-[0.18em] text-white ${compact ? "text-[8px]" : "text-[10px]"}`}>
            AVC <i className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_#34d399]" /> Kayıtlı
          </span>
          {!compact && <span className="mt-1 block whitespace-nowrap text-[8px] font-bold tracking-[0.06em] text-[#9beaf2]">Dijital ekosistem kimliği</span>}
        </span>
      </summary>
      <div className={`absolute z-[70] mt-3 w-[min(330px,calc(100vw-1.5rem))] overflow-hidden rounded-[24px] border border-[#1d4a58] bg-[#071b22] shadow-[0_30px_90px_rgba(4,18,24,0.34)] ${mobile ? "left-0 max-w-full" : "left-0"}`}>
