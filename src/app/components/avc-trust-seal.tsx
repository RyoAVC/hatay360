...
export function AvcTrustSeal({ mobile = false, siteName = "Bu dijital proje", hubUrl = "https://hub.avcieticaret.com" }: AvcTrustSealProps) {
  return (
    <details className={`group relative ${mobile ? "w-full" : ""}`}>
      <summary aria-label="AVC doğrulanmış ekosistem bilgilerini aç" className={`relative flex cursor-pointer list-none items-center overflow-hidden border border-[#173e4b] bg-[#071b22] text-white shadow-[0_8px_24px_rgba(7,27,34,0.18)] transition hover:border-[#29bfd4] [&::-webkit-details-marker]:hidden ${mobile ? "w-full justify-center gap-3 rounded-2xl px-4 py-3" : "gap-2.5 rounded-xl px-2.5 py-2"}`}>
        <span className="absolute inset-y-0 right-0 w-20 bg-[radial-gradient(circle_at_right,rgba(0,168,196,0.24),transparent_70%)]" />
        <span className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/8 text-[#70dce9]"><Fingerprint className="h-4 w-4" /><BadgeCheck className="absolute -bottom-1 -right-1 h-3.5 w-3.5 fill-emerald-400 text-[#071b22]" /></span>
        <span className="relative leading-none"><span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.21em] text-white">AVC <i className="h-1 w-1 rounded-full bg-emerald-400 shadow-[0_0_8px_#34d399]" /> Kayıtlı</span><span className="mt-1 block whitespace-nowrap text-[7px] font-bold tracking-[0.08em] text-white/48">Dijital ekosistem kimliği</span></span>
      </summary>
