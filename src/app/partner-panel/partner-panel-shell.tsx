      ...
      <div className="relative flex min-h-screen flex-col lg:flex-row">
        ...
        <main className="min-w-0 flex-1">
          <div className="sticky top-0 z-20 hidden h-[74px] items-center justify-between border-b border-white/[0.06] bg-[#090b14]/75 px-8 backdrop-blur-2xl lg:flex xl:px-10">
            ...
            <div className="flex items-center gap-3"><span className="inline-flex items-center gap-2 rounded-full border border-emerald-300/15 bg-emerald-400/[0.07] px-3 py-2 text-[10px] font-black text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300 shadow-[0_0_10px_#6ee7b7]" /> Sistem aktif</span><button type="button" onClick={() => onTabChange("support")} aria-label={supportUnread > 0 ? `${supportUnread} okunmamış destek mesajını aç` : "Destek merkezini aç"} title="Destek merkezini aç" className="relative grid h-10 w-10 place-items-center rounded-xl border border-white/[0.07] bg-white/[0.035] text-slate-400 outline-none transition hover:border-teal-300/30 hover:bg-teal-400/10 hover:text-white focus-visible:ring-2 focus-visible:ring-teal-300/60"><Bell className="h-4 w-4" />{supportUnread > 0 ? <span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-rose-300" /> : null}</button></div>
          </div>
