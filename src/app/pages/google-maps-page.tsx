  ...
  return (
    <>
      ...
      <section className="border-y border-[#dce9e5] bg-[#f3f9f7]">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-14 sm:px-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
          <div><span className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-[#12865f]"><Star className="h-4 w-4" /> İtibar yönetimi</span><h2 className="mt-3 text-[30px] font-black tracking-[-0.04em] text-[#173d31]">Yorumları güvenli ve kalıcı biçimde yönetin.</h2><p className="mt-4 text-[14px] leading-relaxed text-[#5b7168]">Olumlu deneyim yaşayan gerçek müşterilere yorum bağlantısı ulaştırır, tüm yorumlara marka dilinde yanıt planı kurarız. Google politikasını ihlal eden yorumları kanıtlarıyla raporlar ve itiraz sürecini takip ederiz.</p>
            <Link to="/araclar/yorum-cevabi" className="mt-4 inline-flex items-center gap-2 text-[12px] font-black text-[#12865f]">Yorum cevap şablonu <ArrowRight className="h-4 w-4" /></Link><ul className="mt-5 space-y-2">{["Gerçek müşteriye QR ve kısa yorum bağlantısı", "Olumlu ve olumsuz yorumlara yanıt şablonları", "Spam ve politika ihlalinde kanıtlı raporlama", "Sahte yorum veya silme garantisi yok; hesap güvenliği öncelikli"].map((item) => <li key={item} className="flex items-start gap-2 text-[12px] font-bold text-[#405e53]"><CircleCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#12865f]" />{item}</li>)}</ul></div>
      ...
      <section className="mx-auto max-w-5xl px-5 pb-8 sm:px-8">
        <h2 className="text-[24px] font-black text-[#173d31]">Sık sorulanlar</h2>
        <p className="mt-2 text-[14px] text-[#5b7168]">Kayıt süresi, sahte yorum ve harita SEO’su — kısa cevaplar.</p>
        <div className="mt-5 space-y-2">
          {MAPS_FAQS.map((faq) => (
            <details key={faq.q} className="rounded-2xl border border-[#dce9e5] bg-white p-4 open:border-[#b7e4d4]">
              <summary className="cursor-pointer text-[15px] font-semibold text-[#173d31]">{faq.q}</summary>
              <p className="mt-2 text-[14px] leading-relaxed text-[#5b7168]">{faq.a}</p>
            </details>
          ))}
        </div>
      </section>
