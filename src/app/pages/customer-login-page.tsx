  ...
  return (
    <div className="grid min-h-screen bg-[#061a20] lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <div className="flex items-center justify-center p-5 sm:p-10">
        <form onSubmit={submit} className="w-full max-w-md rounded-[30px] border border-white/10 bg-white p-7 shadow-[0_28px_90px_rgba(0,0,0,0.28)] sm:p-9">
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#e9f8fa] text-[#008eaa]"><LockKeyhole className="h-5 w-5" /></span><h2 className="mt-6 text-[28px] font-black tracking-[-0.04em] text-[#102b35]">Müşteri paneli girişi</h2><p className="mt-2 text-[12px] leading-relaxed text-[#71818a]">Hatay360 tarafından oluşturulan müşteri hesabınızla giriş yapın.</p>
          {error && <p className="mt-4 rounded-xl bg-red-50 px-3 py-2 text-[11px] font-bold text-red-700">{error}</p>}
          <label className="mt-6 block text-[11px] font-black text-[#425965]">E-posta<input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} className="mt-2 w-full rounded-xl border border-[#dbe6ea] px-4 py-3 text-[13px] outline-none focus:border-[#00a8c4]" /></label>
          <label className="mt-4 block text-[11px] font-black text-[#425965]">Şifre<input type="password" required value={password} onChange={(event) => setPassword(event.target.value)} className="mt-2 w-full rounded-xl border border-[#dbe6ea] px-4 py-3 text-[13px] outline-none focus:border-[#00a8c4]" /></label>
          ...
          <p className="mt-5 text-center text-[10px] font-semibold text-[#87959c]">Hesabınız yoksa Hatay360 temsilcinizden erişim isteyin.</p>
        </form>
      </div>
    </div>
  );
